...
public class SpacingIngestionEngineTests : IDisposable
{
    ...
    {
        var (metadata, engine, _, _) = MakeEngine(worldSize: 100, gridDivisions: 4); // root cellSize = 25
        ...
        long childId = root.Children[octant];
        Assert.NotEqual(NodeRecord.NoneId, childId);
    ...
    {
        var (metadata, engine, warnings, _) = MakeEngine(worldSize: 2000, gridDivisions: 8); // root cellSize = 250, bounds +-1000
    ...
    {
        var (metadata, engine, _, nodeStore) = MakeEngine(worldSize: 1000, gridDivisions: 8);
        ...
        {
            foreach (var p in nodeStore.ReadAll(nodeId))
                Assert.True(node.Bbox.Contains(p), $"point ({p.X},{p.Y},{p.Z}) not contained in node {nodeId}'s bbox");
        });
    }
    ...
    {
        // MaxResidentNodes = 1 forces a page-out/reload on almost every
        ...
        var (metadata, engine, _, _) = MakeEngine(worldSize: 100, gridDivisions: 4, maxResidentNodes: 1);
        ...
        var root = metadata.Get(engine.RootId);
        Assert.Equal(1, root.PointCount); // root's cell was never re-accepted after reload
        Assert.Equal(3, SumPointCounts(metadata, engine.RootId)); // nothing lost or double-counted across the tree
    }

    [Fact]
    public void SumOfPointCountsAcrossAllNodes_EqualsInputCount_WhenNothingIsDropped()
    {
        var (metadata, engine, warnings, _) = MakeEngine(worldSize: 100_000, gridDivisions: 16);
        ...
        for (int batch = 0; batch < 20; batch++)
        {
            ...
            Assert.Equal(total, SumPointCounts(metadata, engine.RootId));
        }
        Assert.Empty(warnings);
    }
    ...
    public void ExactDuplicatePoints_HitMaxDepthAndAreDroppedInsteadOfHanging()
    {
        ...
        var (metadata, engine, warnings, _) = MakeEngine(worldSize: 1.0, gridDivisions: 4, maxSplitDepth: maxSplitDepth);
        ...
        Assert.True(SumPointCounts(metadata, engine.RootId) <= maxSplitDepth + 1);
    }

    [Fact]
    public void PointOutsideWorldBounds_IsDroppedWithWarning()
    {
        var (metadata, engine, warnings, _) = MakeEngine(worldSize: 100, gridDivisions: 4);
        ...
        Assert.Equal(0, metadata.Get(engine.RootId).PointCount);
    }

    private (InMemoryNodeMetadataStore metadata, SpacingIngestionEngine engine, List<string> warnings, NodePointFileStore nodeStore) MakeEngine(
        double worldSize, int gridDivisions, int maxSplitDepth = 60, int maxResidentNodes = 4096)
    {
        ...
        var options = new SpacingIngestionOptions
        {
            ...
            MaxResidentNodes = maxResidentNodes,
        ...
        return (metadata, engine, warnings, nodeStore);
    }

    private static long SumPointCounts(InMemoryNodeMetadataStore metadata, long nodeId)
    {
        long total = 0;
        WalkAll(metadata, nodeId, (id, node) => total += node.PointCount);
        return total;
    }
